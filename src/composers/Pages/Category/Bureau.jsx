...
import bureau from './../../../assets/Bureau/Bu'; // Assurez-vous que ce chemin est correct
...
export default function LaCuisine() {
  ...
  const handleImageClick = (id) => {
    const index = bureau.findIndex((item) => item.id === id);
  ...
  const nextImage = () => {
    setSelectedIndex((prevIndex) => (prevIndex + 1) % bureau.length);
  };

  const prevImage = () => {
    setSelectedIndex((prevIndex) => (prevIndex - 1 + bureau.length) % bureau.length);
  };

  // Filter items based on the search term
  const filteredBureau = bureau.filter((item) =>
      ...
      {selectedIndex !== null && (
        <div style={modalStyles.overlay} onClick={handleCloseModal}>
          <div style={modalStyles.modal} onClick={(e) => e.stopPropagation()}>
            <p style={modalStyles.imageId}>{bureau[selectedIndex].id}</p>
            <img src={bureau[selectedIndex].name} alt="Selected bureau" style={modalStyles.image} />
