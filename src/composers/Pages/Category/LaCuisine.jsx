...
import cuisine from './../../../assets/Cuisine/LaCuisine'; // Assurez-vous que ce chemin est correct
...
export default function LaCuisine() {
  ...
  const handleImageClick = (id) => {
    const index = cuisine.findIndex((item) => item.id === id);
  ...
  const nextImage = () => {
    setSelectedIndex((prevIndex) => (prevIndex + 1) % cuisine.length);
  };

  const prevImage = () => {
    setSelectedIndex((prevIndex) => (prevIndex - 1 + cuisine.length) % cuisine.length);
  };

  // Filter items based on the search term
  const filteredcuisine = cuisine.filter((item) =>
      ...
      {selectedIndex !== null && (
        <div style={modalStyles.overlay} onClick={handleCloseModal}>
          <div style={modalStyles.modal} onClick={(e) => e.stopPropagation()}>
            <p style={modalStyles.imageId}>{cuisine[selectedIndex].id}</p>
            <img src={cuisine[selectedIndex].name} alt="Selected cuisine" style={modalStyles.image} />
